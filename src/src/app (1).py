...
with tab2:
    st.subheader(f"🚨 Auditoría de Anomalías — {modo_periodo} {per_b} vs {modo_periodo} {per_a} (Base)")
    st.caption(f"Compara los viajes de **{modo_periodo} {per_b}** (periodo actual) contra la tarifa media de **{modo_periodo} {per_a}** (periodo base).")
    ...
    if df_bv.empty:
        ...
        # --- Métricas con referencia explícita ---
        c_m1, c_m2 = st.columns(2)
        with c_m1:
            st.metric(f"Tarifa media base — {modo_periodo} {per_a} (referencia)", f"${media_ref:,.2f}", help=f"Promedio FLETE FACTURA / viaje en {modo_periodo} {per_a} ({viajes_a} viajes)")
        with c_m2:
            st.metric(f"Viajes de {modo_periodo} {per_b} por encima de media de {per_a}", f"{len(df_show)} de {len(df_b_grouped)}", delta=f"{len(df_show)/len(df_b_grouped)*100:.1f}%" if len(df_b_grouped)>0 else None, help=f"Cuántos viajes de {modo_periodo} {per_b} superan la media de {modo_periodo} {per_a} (${media_ref:,.2f})")
        st.caption(f"Referencia: ${media_ref:,.2f} = tarifa media de **{modo_periodo} {per_a}** · Evaluado: **{modo_periodo} {per_b}** ({len(df_b_grouped)} viajes)")
        ...
        if not df_show.empty:
            ...
            st.markdown(f"**Detalle:** viajes de **{modo_periodo} {per_b}** que superan la media de **{modo_periodo} {per_a}** (${media_ref:,.2f})")
            st.dataframe(df_vis[cols_orden], use_container_width=True, hide_index=True)
            # Export
            excel_audit = to_excel_bytes({"Auditoria": df_show, "Referencia": pd.DataFrame([{"Periodo Base": f"{modo_periodo} {per_a}", "Tarifa Media Base": media_ref, "Periodo Auditado": f"{modo_periodo} {per_b}", "Viajes Auditados": len(df_b_grouped), "Viajes Encima Media": len(df_show)}])})
            st.download_button("📥 Descargar auditoría (Excel)", data=excel_audit, file_name=f"LogiSense_Auditoria_{modo_periodo}_{per_b}_vs_{per_a}_MediaBase.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            # Gráfico de dispersión Flete vs KG para outliers
            fig_sc = px.scatter(df_b_grouped, x='KG MOVIDOS', y='FLETE FACTURA', color='RUTA', hover_data=['ID_VIAJE_UNICO','Unidad'], title=f"Flete vs KG — {modo_periodo} {per_b} vs media {modo_periodo} {per_a} (${media_ref:,.2f})")
            # resalta outliers
            if not df_show.empty:
                fig_sc.add_trace(go.Scatter(x=df_show['KG MOVIDOS'] if 'KG MOVIDOS' in df_show.columns else df_show['FLETE FACTURA'], y=df_show['FLETE FACTURA'], mode='markers', marker=dict(size=14, line=dict(width=2,color='red'), color='rgba(255,0,0,0.15)'), name=f'Por encima media {per_a}'))
            st.plotly_chart(fig_sc, use_container_width=True)
        else:
            st.success(f"✅ No se encontraron viajes de {modo_periodo} {per_b} por encima de la tarifa media de {modo_periodo} {per_a} (${media_ref:,.2f}).")
